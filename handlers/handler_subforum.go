...
func HandlerSubforum(w http.ResponseWriter, r *http.Request) {
	...
	err := database.GetDB().QueryRow(query, subforumID).Scan(&subforum.ID, &subforum.Name, &subforum.Description, &subforum.CreatedBy, &subforum.CreatedAt, &subforum.UpdatedAt)
	if err != nil {
